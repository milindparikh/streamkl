% Initial philosophy from 
% http://www.trapexit.org/A_fast_web_server_demonstrating_some_undocumented_Erlang_features


-module(streamklw3_server).
...
start_link(Port) when is_integer(Port) ->
    Name = list_to_atom(lists:flatten(io_lib:format("streamklw3_~w", [Port]))),
...
init(Port) ->
	...
	{ok, Listen_socket} ->
	    ...
	    Pid = streamklw3_socket_server:start_link(self(), Listen_socket, Port),
...
handle_cast({create, _Pid}, #state{listen_socket = Listen_socket} = State) ->
    New_pid = streamklw3_socket_server:start_link(self(), Listen_socket, State#state.port),
